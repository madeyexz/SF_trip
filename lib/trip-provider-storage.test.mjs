...
describe('trip provider storage hardening', () => {
  ...
  it('persists lightweight map search preferences without storing search results', async () => {
    ...
    const source = await readFile(preferencesPath, 'utf-8');

    assert.equal(source.includes('window.localStorage.getItem(MAP_SEARCH_PREFERENCES_STORAGE_KEY)'), true);
    assert.equal(source.includes('window.localStorage.setItem('), true);
    assert.equal(source.includes('JSON.stringify({ query, scope, sort })'), true);
