...
describe('layout script hardening', () => {
  ...
  it('loads Vercel analytics only in production so development CSP stays quiet', async () => {
    const layoutPath = path.join(process.cwd(), 'app', 'layout.tsx');
    const source = await readFile(layoutPath, 'utf-8');

    assert.equal(source.includes("import { Analytics } from '@vercel/analytics/next'"), true);
    assert.match(source, /const shouldRenderAnalytics = process\.env\.NODE_ENV === 'production';/);
    assert.match(source, /\{shouldRenderAnalytics \? <Analytics \/>\s*: null\}/);
  });
});
