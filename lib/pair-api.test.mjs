...
describe('pair-api', () => {
  ...
  it('validates leave action', () => {
    const payload = parsePairActionBody({ action: 'leave' });
    assert.equal(payload.ok, true);
    assert.equal(payload.action, 'leave');
    assert.equal(payload.roomCode, '');
  });
  ...
  it('rejects unsupported actions', () => {
    ...
    assert.equal(payload.ok, false);
    assert.equal(payload.error, 'Unsupported action. Use "create", "join", or "leave".');
  });
});
