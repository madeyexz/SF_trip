...
describe('syncEvents with deterministic mocked feeds', () => {
  ...
  it('loads room-scoped sources and appends fallback base sources for missing types', async () => {
    process.env.CONVEX_URL = 'https://mock.convex.cloud';
    ...
    assert.equal(
      sources.some((source) => source.sourceType === 'event' && source.url === CALENDAR_URL_1),
      false,
      'custom room event sources should replace default event source URLs'
    );
  });
